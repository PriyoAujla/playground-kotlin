...
abstract class Table<in T> {
    ...
    private fun update(newValues: Set<ColumnValue>) {
        ...
        if (id != null && columns.contains(id.column)) {
            val sqlUpdate = """
                UPDATE
                    SET ${newValues.filter { it.column !is AutoGeneratedKeyColumn<*> }.map { "${it.column.name.value} = ${it.valueAsSqlString}" }.joinToString(", ")}
                FROM $name
                    WHERE ${id.column.name.value} = ${id.valueAsSqlString}
            """.trimIndent()
            println(sqlUpdate)
        } else {
    ...
    private fun delete(columnValue: ColumnValue) {
        ...
        if (isAnIdentifier && tableHasSuchAColumn) {
            val sqlDelete = """
                DELETE FROM $name WHERE ${columnValue.column.name.value} = ${columnValue.valueAsSqlString}
            """.trimIndent()
